...
class DFRBlock(nn.Module):
    """
    Classification calibration block.

    Note:
        The class name is kept as DFRBlock to avoid extra registration.
        Internally, this block is used for classification calibration,
        not box regression refinement.
    """

    def __init__(self, c, beta=0.05, reduction=4):
        super().__init__()
        ...
        # Channel-wise classification calibration gate
        ...
        # Lightweight local classification refinement
    ...
    def forward(self, x):
        g = self.gate(x)
        r = self.refine(x)
        return x + self.beta * torch.tanh(self.alpha) * g * r


class DFRDetect(Detect):
    """
    Classification-calibrated Detect head.

    Note:
        The class name is kept as DFRDetect to reuse the previous registration.
        This version does NOT modify the box regression branch.
        It only inserts DFRBlock into the classification branch cv3.

    Args:
        nc: number of classes
        beta: fixed residual scale
        dfr_layers: indices of detection scales using classification calibration.
                    [1, 2] means P4 and P5.
                    scale index: 0=P3, 1=P4, 2=P5
        reg_max: DFL regression bins, passed by parse_model
        end2end: end-to-end flag, passed by parse_model
        ch: input channels, passed by parse_model
    """

    def __init__(self, nc=80, beta=0.05, dfr_layers=(1, 2), reg_max=16, end2end=None, ch=()):
        # Your current tasks.py passes:
        # [nc, beta, dfr_layers, reg_max, end2end, ch]
        super().__init__(nc, reg_max, end2end, ch)

        self.beta = float(beta)

        if isinstance(dfr_layers, int):
            dfr_layers = [dfr_layers]
        self.dfr_layers = set(dfr_layers)

        # Same classification channel rule as original Detect
        c3 = max(ch[0], min(self.nc, 100))

        # Replace ONLY classification branch cv3.
        # Box regression branch cv2 remains unchanged from original Detect.
        self.cv3 = nn.ModuleList(
            nn.Sequential(
                Conv(x, c3, 3),
                Conv(c3, c3, 3),
                DFRBlock(c3, beta=self.beta) if i in self.dfr_layers else nn.Identity(),
                nn.Conv2d(c3, self.nc, 1),
            )
            for i, x in enumerate(ch)
        )

        # Keep end2end compatibility
...
class FeatureSplitter(nn.Module):
    """ 通道切分插件：在卷积之前，悄悄把混合的特征一分为二 """

    def __init__(self, mode, ch_cls):
        super().__init__()
        self.mode = mode
        self.ch_cls = ch_cls

    def forward(self, x):
        # x 包含了分类和回归特征的拼接，我们在通道维度 (dim=1) 进行无损切割
        if self.mode == 'cls':
            return x[:, :self.ch_cls, :, :]  # 取前一半给分类
        else:
            return x[:, self.ch_cls:, :, :]  # 取后一半给回归


class CERADetect(Detect):
    """ CERA 专用双流解耦检测头 """
    ...
    def __init__(self, *args, **kwargs):
        ...
        c2 = max((16, ch_reg[0] // 4, self.reg_max * 4))
        c3 = max(ch_cls[0], min(self.nc, 100))

        # 【核心黑科技】：重写 cv2 和 cv3，在它们的最前面加上我们的切分插件！
        # 这样父类在正常调用 cv2 时，会自动触发 FeatureSplitter，只计算该算的特征
        self.cv2 = nn.ModuleList(
            nn.Sequential(
                FeatureSplitter('reg', ch_cls[i]),  # 专门提取回归特征
                Conv(ch_reg[i], c2, 3),
                Conv(c2, c2, 3),
        ...
        self.cv3 = nn.ModuleList(
            nn.Sequential(
                FeatureSplitter('cls', ch_cls[i]),  # 专门提取分类特征
                Conv(ch_cls[i], c3, 3),
                Conv(c3, c3, 3),
                nn.Conv2d(c3, self.nc, 1)
            ) for i in range(nl)
    ...
    def forward(self, x):
        # x 是我们传入的 6 个特征层 [P3_cls, P4_cls, P5_cls, P3_reg, P4_reg, P5_reg]
        # 我们把同尺度的 cls 和 reg 在通道维度拼起来，变成 3 个特征层
        x_combined = [torch.cat([x[i], x[i + self.nl]], dim=1) for i in range(self.nl)]

        # 把伪装好的 3 个特征层交给父类
        # 父类会浑然不知地把它们交给我们的 cv2 和 cv3，而里面的 FeatureSplitter 会完美拆解它们！
        return super().forward(x_combined)
