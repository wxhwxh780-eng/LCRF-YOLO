...
from ultralytics.nn.modules.block import DynamicC2PSA
...
#from .sfa_custom import E_IAN, G_MANet, ACF
    ...
    YOLOESegment,
    YOLOESegment26,
    v10Detect, EMCADLite, MFMSC3k2, DSC3k2, BGC3k2, HLGFBlock, HLGFBlock, ADC3k2, MRC3k2, C2SCA,DCNC3k2,  # C2MGA
)
        ...
        if isinstance(m, Detect):  # includes all Detect subclasses like Segment, Pose, OBB, YOLOEDetect, YOLOESegment
            ...
            def _forward(x):
                ...
                if isinstance(output, dict):
                    return output["feats"] if "feats" in output else output["one2many"]
...
class RTDETRDetectionModel(DetectionModel):
    ...
    def loss(self, batch, preds=None):
        ...
        if not hasattr(self, "criterion"):
            self.criterion = self.init_criterion()
            print("Using criterion:", type(self.criterion))
        img = batch["img"]
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            # 多输入张量相加，物理通道数与主支路保持一致
            c2 = ch[f[0]]
            args = []
        elif m is ACF_Module:           #########wo jiad
            ...
            args = [c1, *args]
            c2 = c1
        elif m is GRCSFusion:                        ###########ye也是我加的
            assert isinstance(f, list) and len(f) == 2, "GRCSFusion requires two input layers."
            c_source, c_target = ch[f[0]], ch[f[1]]
            args = [c_source, c_target, *args]
            c2 = c_source + c_target
        elif m is ACF:    #####这是我加的
            c1 = sum(ch[x] for x in f)
            c2 = args[0]
            c2 = make_divisible(min(c2, max_channels) * width, 8)
            args = [c1, c2]
        elif m is CAFM_Fusion: #下面6行是我加的
            ...
            if m in {Detect,DFRDetect, YOLOEDetect, Segment, Segment26, YOLOESegment, YOLOESegment26, Pose, Pose26, OBB, OBB26}:
                m.legacy = legacy
        elif m is v10Detect:
        ...
        try:
            ...
            print(f"崩溃模块: {m}")
            print(f"传入参数: {args}")
            print(f"请检查这个模块的 __init__ 函数，或者直接给它加上 *args, **kwargs")
            raise e
    ...
    if isinstance(model, torch.nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, (Segment, YOLOESegment)):
                ...
                return "obb"
            elif isinstance(m, (Detect,DFRDetect, WorldDetect, YOLOEDetect, v10Detect)):
                return "detect"
