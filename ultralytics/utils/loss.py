...
def shape_iou_xyxy(box1, box2, scale=0.0, eps=1e-7):
    ...
    ww = 2 * torch.pow(w2, scale) / (torch.pow(w2, scale) + torch.pow(h2, scale) + eps)
    hh = 2 * torch.pow(h2, scale) / (torch.pow(w2, scale) + torch.pow(h2, scale) + eps)

    cw = torch.max(b1_x2, b2_x2) - torch.min(b1_x1, b2_x1)
    ch = torch.max(b1_y2, b2_y2) - torch.min(b1_y1, b2_y1)
    c2 = cw ** 2 + ch ** 2 + eps

    center_distance_x = (c2x - c1x) ** 2
    center_distance_y = (c2y - c1y) ** 2
...
class BboxLoss(nn.Module):
    ...
    def forward(
        ...
        #iou = shape_iou_xyxy(pred_bboxes[fg_mask], target_bboxes[fg_mask], scale=0.0)
        iou_ciou = bbox_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, CIoU=True)
        #iou_shape = shape_iou_xyxy(pred_bboxes[fg_mask], target_bboxes[fg_mask], scale=0.0)
        #iou = 0.8 * iou_ciou + 0.2 * iou_shape
...
###############
class MDABboxLoss(BboxLoss):
    """
    MDABboxLoss: Morphology-aware Dynamic Alignment Bbox Loss

    只作用于 box loss 和 dfl loss，不改 cls loss。
    适合 NEU-DET / GC10-DET / PCB 这类工业缺陷检测数据。
    """
    ...
    def __init__(
        ...
        # 关键修复：Ultralytics 当前 BboxLoss 不一定保存 reg_max
    ...
    def _get_stride_for_fg(self, stride_tensor, target_bboxes, fg_mask):
        if stride_tensor is None:
            return None

        if stride_tensor.ndim == 2:
            stride = stride_tensor.view(1, -1, 1).expand(target_bboxes.shape[0], -1, 1)
        elif stride_tensor.ndim == 3:
            if stride_tensor.shape[0] == 1:
                stride = stride_tensor.expand(target_bboxes.shape[0], -1, -1)
            else:
                stride = stride_tensor
        else:
            return None

        return stride[fg_mask].squeeze(-1)

    def _mda_weight(self, target_bboxes_fg, iou, stride_fg=None):
        eps = 1e-6

        x1, y1, x2, y2 = target_bboxes_fg.unbind(dim=-1)
        ...
        # 小目标因子
        sqrt_area = torch.sqrt((w_pix * h_pix).clamp(min=eps))
        size_factor = torch.exp(-sqrt_area / self.tau)

        # 细长形态因子
        aspect = torch.maximum(w_pix / (h_pix + eps), h_pix / (w_pix + eps))
        ratio_factor = torch.tanh(torch.log(aspect + eps))

        # 困难定位因子
        iou_for_hard = iou.detach().clamp(min=0.0, max=1.0)
        hard_factor = (1.0 - iou_for_hard).pow(self.gamma)

        mda_weight = (
            1.0
            + self.lambda_s * size_factor
            + self.lambda_r * ratio_factor
            + self.lambda_h * hard_factor
        )

        mda_weight = mda_weight.clamp(min=self.min_weight, max=self.max_weight)

        if self.normalize and mda_weight.numel() > 0:
            mda_weight = mda_weight / mda_weight.mean().clamp(min=eps)

        return mda_weight, size_factor, ratio_factor, hard_factor
    ...
    def forward(
        ...
        if fg_mask.sum() == 0:
            return pred_dist.sum() * 0.0, pred_dist.sum() * 0.0
        ...
        stride_fg = self._get_stride_for_fg(stride_tensor, target_bboxes, fg_mask)

        mda_weight, size_factor, ratio_factor, hard_factor = self._mda_weight(
            target_bboxes_fg=target_bboxes_fg,
            iou=iou,
            stride_fg=stride_fg,
        )

        mda_weight = mda_weight.unsqueeze(-1)

        # box loss
        loss_iou = ((1.0 - iou).unsqueeze(-1) * weight * mda_weight).sum() / target_scores_sum

        # dfl loss
        if self.reg_max > 1 and self.dfl_loss is not None:
        ...
        if self.debug and self.training:
            if self.debug_count % self.print_interval == 0:
                print(
                    f"[MDA-Loss Debug] "
                    f"mda_mean={mda_weight.mean().item():.4f}, "
                    f"mda_min={mda_weight.min().item():.4f}, "
                    f"mda_max={mda_weight.max().item():.4f}, "
                    f"size={size_factor.mean().item():.4f}, "
                    f"ratio={ratio_factor.mean().item():.4f}, "
                    f"hard={hard_factor.mean().item():.4f}, "
