...
class TDAug:
    """
    TDAug-v3-target: Texture-Direction Aware Augmentation
    类别选择版方向纹理增强

    只增强 target_classes 指定的类别。
    NEU-DET 默认：
        0: crazing
        4: rolled-in_scale
        5: scratches
    ...
    def __init__(
        ...
        self.dre_strength = dre_strength
        self.dta_strength = dta_strength
        self.rti_strength = rti_strength
        self.target_classes = None if target_classes is None else set(target_classes)
    ...
    def __call__(self, labels):
        ...
        boxes = self._get_boxes_xyxy(instances, h, w)
        classes = self._get_classes(labels)

        # 类别筛选：只保留 target_classes 的 bbox
        if self.target_classes is not None:
            if len(boxes) == 0 or len(classes) != len(boxes):
                return labels

            keep = np.array([int(c) in self.target_classes for c in classes], dtype=bool)
            boxes = boxes[keep]
        ...
        valid_boxes = self._filter_boxes(boxes, h, w)
        if len(valid_boxes) == 0:
            return labels

        random.shuffle(valid_boxes)
        valid_boxes = valid_boxes[: self.max_boxes]

        # target 版本只做 DRE / DTA，先不做 RTI，避免影响 patches/inclusion
        mode = random.choices(
            ["dre", "dta"],
            weights=[0.55, 0.45],
            k=1,
        ...
        for box in valid_boxes:
            ...
            theta = self._estimate_main_direction(roi)

            if mode == "dre":
                roi_aug = self._directional_ridge_enhance(
                    roi,
                    theta,
                    strength=self.dre_strength,
                )
            else:
                roi_aug = self._directional_texture_attenuate(
                    roi,
                    theta,
                    strength=self.dta_strength,
                )

            img_f[y1:y2, x1:x2] = roi_aug

        labels["img"] = np.ascontiguousarray(np.clip(img_f, 0, 255).astype(np.uint8))
        return labels

    def _get_classes(self, labels):
        cls = labels.get("cls", None)
        if cls is None:
            return np.zeros((0,), dtype=np.int64)

        try:
            cls = np.asarray(cls).reshape(-1).astype(np.int64)
            return cls
        except Exception:
            return np.zeros((0,), dtype=np.int64)

    def _get_boxes_xyxy(self, instances, h, w):
        ...
        try:
            ...
            fmt = "xyxy"
            if hasattr(instances, "_bboxes") and hasattr(instances._bboxes, "format"):
                fmt = instances._bboxes.format

            normalized = getattr(instances, "normalized", False)
    ...
    def _filter_boxes(self, boxes, h, w):
        valid = []

        for b in boxes:
            x1, y1, x2, y2 = b.astype(np.int32).tolist()
            ...
            if bw >= self.min_box_size and bh >= self.min_box_size:
                valid.append((x1, y1, x2, y2))

        return valid

    def _estimate_main_direction(self, roi):
        gray = cv2.cvtColor(np.clip(roi, 0, 255).astype(np.uint8), cv2.COLOR_BGR2GRAY)
        ...
        theta = 0.5 * np.arctan2(2.0 * gxy, gxx - gyy + 1e-6)
        return theta

    def _directional_kernel(self, theta, k=7):
    ...
    def _directional_ridge_enhance(self, roi, theta, strength=0.12):
        kernel = self._directional_kernel(theta, k=7)
        smooth = cv2.filter2D(roi, -1, kernel)
        detail = roi - smooth
        out = roi + strength * detail
        return np.clip(out, 0, 255)

    def _directional_texture_attenuate(self, roi, theta, strength=0.08):
        kernel = self._directional_kernel(theta, k=7)
        smooth = cv2.filter2D(roi, -1, kernel)

        out = roi * (1.0 - strength) + smooth * strength

        mean = out.mean(axis=(0, 1), keepdims=True)
        out = mean + (out - mean) * (1.0 - 0.35 * strength)
...
def v8_transforms(dataset, imgsz: int, hyp: IterableSimpleNamespace, stretch: bool = False):
    ...
    return Compose(
        [
            ...
            # TDAug: Texture-Direction Aware Augmentation
            # 训练侧方向纹理感知增强，不改变 bbox，不增加推理成本
            # TDAug(
            #     p=0.25,
            #     max_boxes=2,
            #     min_box_size=8,
